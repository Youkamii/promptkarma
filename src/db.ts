...
export async function upsertCard(r: CardRow): Promise<void> {
  const db = sql();
  await db`
    INSERT INTO cards (username, profanity, competence, prompts, prompts_per_swear, updated_at)
    VALUES (${r.username}, ${r.profanity}, ${r.competence}, ${r.prompts}, ${r.promptsPerSwear}, now())
...
export async function getCard(username: string): Promise<CardRow | null> {
  ...
  const rows = (await db`
    SELECT username, profanity, competence, prompts,
           prompts_per_swear AS "promptsPerSwear"
